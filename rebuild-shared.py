#!/usr/bin/env python3
"""Rebuild the NyaaChat shared-character backend (nyaachat-shared, port 5107).

Independent of the frontend rebuild. See .docs/shared-character-system.md.

Usage:
  python rebuild-shared.py            # Build with cache (default)
  python rebuild-shared.py --no-cache # Full rebuild without cache
"""
...
def main() -> None:
    ...
    # 1. Ensure external network exists (shared with main compose project)
    ...
    # 2. Build
    if no_cache:
        say("Building shared backend (no cache)...", colour=CYAN)
        run(["docker", "compose", "-f", COMPOSE_FILE, "build", "--no-cache"])
    else:
        say("Building shared backend (using cache)...", colour=CYAN)
        run(["docker", "compose", "-f", COMPOSE_FILE, "build"])

    # 3. Start containers
    say("Starting shared backend...", colour=CYAN)
