...
| 2026-09-17 | **真缺陷（用户症状「MVU 变量刷新了，但渲染的状态栏依旧是初始数据」）：变量通知早于数据落地一拍**。**定位**（新增决定性探针）：向种子会话的 `variables[0]` **预置** `stat_data.白石栞.好感度=777 / 世界.当前场所=P0`，卡片首渲染即读到 `場所 P0`（证明卡片读变量链路与 `getAllVariables()` 的扁平形状都正确；「好感度 100」是 777 被 `_.clamp(…,0,100)` 钳制的显示值，不是兜底），随后经宿主 API 写入 `世界.当前场所=MARKER_PLACE`：**卡片自动重绘的事件计数器 +1（事件链是通的），但 render 读到的仍是 `P0`；再手动 `__nyaCardDispatch` 一次才变成 `MARKER_PLACE`**。**根因**：`ChatInterface` 的 `VariableAdapter.patchMessages` 走 `setMessages`（React 下一帧才反映到 `messagesRef`），而 `writeScopeData` 在 patch 之后**同步**执行 `notifyVariableListeners()`；适配器的 `getSession` 读路径返回的是**裸 `messagesRef.current`**，没有叠加那层「本帧补丁覆盖层」（`varPatchOverlayRef` —— 落盘路径 `persistLive` 一直在用它）⇒ 同步通知到达时，订阅者（前端卡重绘、脚本事件处理器）读到的永远是**写入前**的快照，状态栏因此恒定落后一拍。**修法**：`getSession` 的 live 与 draft 两条分支都改用 `patchArrayOverlay(messagesRef.current, varPatchOverlayRef.current)`。**验证**：同一探针在修复后（build `v12-2400`）**不手动派发**即读到 `場所 MARKER_PLACE`；§9 V1–V8 回归 8 通过 / 0 失败（V2 因 dist 过期 SKIP）。 |
| 2026-09-17 | **初始化窗口改为真正阻塞输入（用户拍板）+ 空聊天不再放行装配**。用户指出：既然有「数据初始化中」的提示机制，就必须**挡住输入**，且应在「到达不会造成初始化失败的状态」后才解除；原来只有一行 `pointer-events:none` 的浮动提示，宿主侧仅靠 `getScriptInitBusy()` 在发送函数里静默 return（按钮与输入框都可用）⇒ 用户可在窗口内抢发消息。**真机日志证据**：v11-1542 那次 MVU 在 `chatReady:false / messages:0` 时装配，直接打出「不存在任何一条消息，退出」，并且**不重试** —— MVU 的 initvar 一旦写下 `initialized_lorebooks` 记名就永久跳过，该会话变量永久为空、状态栏只剩兜底值。**修法三处**：① `ChatComposer` 新增 `scriptInitBusy` prop（`textarea disabled` + placeholder 换初始化文案 + 表单/键盘/按钮三条提交路径早退 + 输入区下方转圈提示），`ChatInterface` 经 `useSyncExternalStore(subscribeScriptInitBusy, getScriptInitBusy)` 订阅并下发，`handleSubmit` 另加二道闸；② 忙碌窗口起点提前到「打开卡片/会话」（等 `chatReady` **之前**），终点由新的 `settleInitBusy()` 判定 —— 会话就绪 + 宿主 iframe 已装配 + **该卡有 MVU 脚本时再等宿主 iframe 内 `window.Mvu` 就位**（没有 MVU 的卡不白锁）；硬上限 20s→60s 仅作兜底；③ 等 `chatReady` 超时（6s→18s）后**不再照装**，改为放弃本次装配并放行输入（会话落地后 `session:changed` 会再触发 mount）；`onScriptResult` 里「第一个脚本返回就解锁」的逻辑删除（脚本返回 ≠ 变量初始化落地）。**验证**（逐秒采样探针）：t+0s `textarea.disabled=true`、浮动提示 `opacity=1`、宿主 `Mvu=undefined`；t+1s `Mvu=object` 就位后 `disabled=false`、提示 `opacity=0` —— 阻塞窗口与解除条件均与设计一致。 |

| 2026-09-17 | **真缺陷（用户症状「回复的消息格式发生错误，导致正则失效，无法替换成状态栏渲染」）：切分器把带 info string 的围栏当成了收尾围栏**。用户表述里的"正则失效"经日志证伪 —— 占位符其实已被替换（`hasPlaceholder:false`），失效的是**切分**：`[frontend-card] 切分结果 {types: null, fenceLangs: ["```","```html","```"]}`。**根因**：`detect.ts` 的注释（第 36–38 行）明确写着「Mirrors GFM's closing fence may not carry an info string」，但实现是 `if (!FENCE_LINE.test(lines[i])) continue;` —— **任何**围栏行都能收尾。真机形态是「正文里一个**未闭合的裸 ```**（美化正则留下的），后面紧跟应用自己插入的 ```html 状态栏围栏」⇒ 那个 ` ```html ` 被当成裸块的收尾吃掉 ⇒ 整条消息一张卡都切不出来，正文与状态栏 HTML 一起漏成气泡纯文本。**修法**：新增 `langOf()`；**收尾围栏必须无 info string**；裸围栏与收尾之间若夹着带标签的围栏行，则判定该裸块未闭合、跳过它让内部的 ` ```html ` 正常开卡片；相邻卡片之间不再产出空白 markdown 段。**验证**：形态断言 **17/17**（§4.2 的既有 16 形态 + 本形态，含"card 体内不得混入围栏行/裸 CSS"的反向断言）；端到端探针（build `v12-2500`）→ 卡片 1 个、卡片内 `CARD_MARKER_9137`、正文 `正文MARKER_4242` 留在气泡、卡片 HTML 源码**不再**漏成正文。 |
| 2026-09-17 | **真隐患（用户症状「点重新生成消息后状态栏渲染出来，但正文不再出现」的候选根因）**：`ChatInterface` 的 `varPatchOverlayRef.current.clear()` 原先写在 **render body** 里。React 的并发渲染 / StrictMode 可能**丢弃**一次渲染，那一刻 overlay 已被清空、而 state 并未提交 ⇒ 读路径（`getSession` → 变量层与脚本侧 `getChatMessages`）看不到刚落地的补丁，脚本会把**旧正文**当成当前内容再写回（MVU 的额外解析路径 `pn`：`o = n.message.trimEnd()` 后追加 `<UpdateVariable>` 与占位符并 `setChatMessages`），从而覆盖新正文。**修法**：`clear()` 移到无依赖的 `useEffect`（只在 commit 之后清空）。**同轮诊断增强**：`__nyaScriptRunnerDiag()` 快照新增 `busy`、每个楼层的 `stat_data` 顶层键 `sdKeys`、正文长度 `len`；初始化窗口结束改以 `warn` 落盘（dev 的 console 收集器只收 warn/error，`info` 不进 devlog —— 这正是上一轮"窗口只有 1s 而用户完全看不到"没能第一时间被发现的原因）。**状态**：detect 修复已确证；本条为候选根因，待用户复测确认。 |
| 2026-09-17 | **真缺陷（用户症状「对话让 MVU 变量更新了，但更新的数值未进入状态栏」）：脚本经 `setChatMessages` 写楼层变量时，变量订阅者收不到通知**。**根因**：MVU 的变量**权威形态**是楼层里的 `variables[0]`，而它大量经 `setChatMessages([{message_id, swipes_data}])` 直接写（initvar 路径、`i(e)` 同步路径），底层走宿主 `api.messages.update(...)` → `VariableAdapter.patchMessages` —— 这条路径**不经过**变量层的 `writeScopeData`，因此**不 notify**，前端卡（状态栏）永远收不到重绘信号。**注意这正是既有探针的盲区**：此前所有验证都用变量层 API（`api.variables.updateVariablesWith`，它自己会 notify），所以"链路通"的结论掩盖了这条真实路径。**修法**：`scopes.ts` 导出 `notifyVariablesChanged()`；`ChatInterface` 的三条适配器路径（`patchMessages` / `patchSession` / `commitSession`）在补丁/字段里**显式出现 `variables`** 时各广播一次（变量层自己的写入会因此多收一次通知 —— 幂等，无害）。**验证（含反向对照）**：探针经 `api.messages.update` 写入 `世界.当前场所 = PLACE_B` —— **去掉那一行通知**时卡片文本停在 `PLACE_A` 而变量层已是 `PLACE_B`（**精确复现用户症状**）；恢复后 +1s 卡片变为 `PLACE_B`（build `v12-2600`）。 |

| 2026-09-17 | **真缺陷（本次问题的真正根因）：`getChatMessages(n)` 违反 ST/JSR 契约 —— 只返回第 n 条，而不是「从第 n 条到最新」**。用户反复报告「MVU 变量更新了，但状态栏数值不变」。**定位**：MVU 的变量更新入口（bundle `Yt(e)`）第一行就是 `const t = getChatMessages(e).at(-1); const n = t.message;` —— 它要的是**最新那条消息**（本轮 `<UpdateVariable>` 就在它的正文里）。我们把它实现成「只取第 n 条」⇒ `.at(-1)` 拿到的是**楼层 e 的正文**；真机上 MVU 拿到 `e=0`（开场白）时，等于每轮都在解析开场白、再把变量写回楼层 0 ⇒ **最新 AI 楼层的变量永远是初值的拷贝**，而前端卡读的正是「最后一个有变量的楼层」⇒ 状态栏恒定显示初值。**证据**：在宿主 iframe 内直接实测 `getChatMessages(0)` = **1 条**（应为 3 条）。**修法**：`predefine.ts` 宿主侧 `normalizeRange()` 的 number 分支改为返回 `[start..last]`；卡片侧 `window.getChatMessages` 同步修正。**同时修掉同一缺陷的残留**：宿主侧 `getAllVariables()` 仍是 `{global,chat,message}` 嵌套壳（卡片侧早已按 ST 扁平契约修正）⇒ 改为返回 message 作用域的扁平快照。**验证**：修复后探针实测 `getChatMessages(0)` = **3 条**（ids `[0,1,2]`）；§9 V1–V8 回归 8 通过 / 0 失败。**方法学**：此前所有探针都走「变量层 API」与「messages.update」，**没有覆盖 MVU 的读取契约** —— 这是本轮第二次栽在「验证没走被测系统真正走的路」上。 |

| 2026-09-17 | **⚠️ 回归 + 回滚（我引入的，用户截图实证）：把 `getChatMessages(n)` 改成「从 n 到最新」导致消息正文被逐层覆盖**。起因是上一行（第九轮）判断"ST 契约应为从 n 到最新"—— **该判断错误**。MVU 的 `Yt(e)` 是「`const t = getChatMessages(e).at(-1)`（取最新）→ 处理 → `setChatMessages([{message_id: e, message}])`（写回楼层 e）」；一旦 `getChatMessages(e)` 返回 `[e..last]`，**每个楼层的事件都会拿最新消息的正文重写自己那一层** ⇒ 首条消息正文丢失、状态栏错位、重新生成为空（用户截图：消息 #0 / #2 的内容只剩 `<StatusPlaceHolderImpl/>`）。**已回滚**宿主侧 `normalizeRange()` 与卡片侧 `window.getChatMessages` 到「number = 只取这一条」，并保留说明性注释。**回滚验证**：`getChatMessages(0/1/2/'latest')` = **1/1/1/1**；派发 `MESSAGE_RECEIVED(2)` 前后三个楼层内容长度不变（30/2/29）。**教训**：`getChatMessages` 与 `setChatMessages` 是**配对使用**的公共契约，改一侧前必须做全局用法普查，不能只盯着某一个调用点的期望。 |
| 2026-09-17 | **变量不更新的真根因（已确证，与宿主无关）：模型输出的更新块缺少 `<JSONPatch>` 标签**。用 MVU 自己的入口取证（新增快照字段 `mvuParse`：调用 `Mvu.parseMessage(最新消息正文, 该楼层变量)`）：`{"textLen":2278,"hasUpdateVariable":true,"hasPatchTag":false,"patchBlock":null,"changed":false}` —— 用户消息里是**裸 JSON 数组**（`… 5 }\n]\n</UpdateVariable>`），而 `Ft` 只认两种写法：`<JSONPatch>…</JSONPatch>` 块、或 `_.set(...)` 类 lodash 命令；裸数组 ⇒ 解析 0 条命令 ⇒ **静默不更新**（`Ft` 每块 `try{…}catch{}`，故零警告）。**为什么看起来"有更新"**：卡的美化正则 `[美化]变量完成` 匹配的是整个 `<UpdateVariable>` 块（不要求 `<JSONPatch>`），所以折叠块能渲染出 patch，而 MVU 一条都没执行。**结论**：该症状属"模型未按卡要求输出格式"（本次为 `gemini-3.7-flash`），修法在提示词/模型侧；宿主侧可选"解析前规范化"但会把补过的标签写回消息（污染原文），需用户拍板。 |

| 2026-09-17 | **真根因（宿主缺陷，已修复并验证）：`SillyTavern.chat` 的 getter 每次读都新建数组与元素 ⇒ MVU 的引用相等守卫恒为 false ⇒ 变量更新被静默放弃**。用户连续多轮报「MVU 变量更新了，但状态栏数值永不变」。**定位链**：① 新增快照字段 `mvuParse`（调用 MVU 自己的 `Mvu.parseMessage(最新消息正文, 该层变量)`）证明**消息格式完全正确**（`patchTagOpenCount/CloseCount=1`、patch 是合法 JSON、无零宽字符），但 `changed:false` 且 `patchOnlyChanged:false`；② MVU 的 `Yt(e)` 靠 `At(x)` 取变量，而 `At` → `Ct(e)` 只在 **`SillyTavern.chat`** 里找「同时含 stat_data 与 schema 的最新楼层」；③ bundle 里 `SillyTavern.chat` **没有任何赋值点** ⇒ 必须由宿主提供；④ 宿主的 `buildSillyTavernShell` 确实提供了 `chat`，但 getter 是 `a.messages.getAll().map(...)` —— **每次读都新建数组与新元素对象**；⑤ 而 MVU 的 `jo` 守卫写的是 `const s = SillyTavern.chat[e]; … SillyTavern.chat[e] === s && …`（**引用相等**）⇒ 恒 false ⇒ `jo` 直接放弃、**不调用 `Yt`**。这解释了全部反常：变量不动、**console 零警告**、`write mid=0` 只来自 initvar。**修法**：`tavernChat()` 按内容指纹（会话 id + 楼层数 + 各楼层 content 长度 + 各楼层 variables 的 hash）缓存，**内容不变时返回同一个数组引用**。**验证**：探针 `sameArray` 修复前 false → 修复后 **true**（`sameElement` 同样 true）；端到端派发 `MESSAGE_RECEIVED(2)` 后楼层 2 = 好感度 85→**86**、性欲 20→**35**、場所 →**同居公寓·卧室**，而楼层 0 保持不变。**教训**：给兼容壳做 "live getter" 时，必须确认调用方是否依赖**引用相等**；"live"（内容最新）不等于"每次新建"。 |

| 2026-09-17 | **真根因（宿主缺陷，已修复并端到端验证）：`identity` 传空快照 ⇒ `{{user}}` 宏被替换成空 ⇒ mvu_zod 的 safeParse 永久失败 ⇒ 变量更新被静默丢弃**。用户连续多轮报「MVU 变量更新了，但状态栏数值永不变」。**完整链条**：`App.tsx` 调 `createScriptHostApi({ identity: { user: "", char: "" } })`（空快照，注释误以为"每次装配读一次即可"）→ `scriptHostImpl` 里 `const { character, identity } = args` 后，`macros.substitute` **闭包捕获该快照**，而它在**每次调用**时读 `identity.user` 来替换 `{{user}}` → `substitudeMacros('{{user}}')` 返回**空字符串** → initvar 条目里的 `'{{user}}':` 键在 `stat_data` 中变成 `""`（真机 `sdKeys` 里那个空键就是它）→ 而 `mvu_zod` 的 `Schema` 用**活值** `__mvuResolveUserKey()`（`getCurrentPersonaName()` = 真实用户名）→ 键缺失且无兜底 → `safeParse` 失败 → `mvu_zod` 的 `mag_command_parsed_for_zod` 处理器把所有命令从数组里 `_.pullAt` 掉、随后 `_ended_for_zod` 清空 ⇒ **MVU 主循环无命令可应用** ⇒ 变量永不更新。**为什么零警告**：`mvu_zod` 的失败告警写作 `return l && s && n('warn', …)`，`l = Boolean($('#mvu_notification_error').prop('checked'))` —— 该复选框默认不在 DOM ⇒ 完全静默。**修法**：`App.tsx` 改为传**活对象（getter）**，`api.identity` 与 `macros.substitute` 同源活值。**验证**：修复前（zod 已注册）`desire` 停 20 且报 `✖ Invalid input: expected object, received undefined → 路径: user`；修复后 `desire 20→25`、无警告。**定位方法（可复用）**：① 在本地把真机会话形态复现出来（先只 seed 开场白让 MVU 把变量初始化到楼层 0，再重建 3 层会话）；② **必须等 `mvu_zod` 注册完成**（它要 import 4 个 npm 模块，约 30s；只等 MVU 会走"无 zod"路径从而误判为已修复）；③ 临时预置并勾选 `#mvu_notification_error`，逼出 zod 的报错原文。 |

| 2026-09-17 | **新增「MVU 变量更新本地自测」能力（脚本 + 方法论文档），把这类问题的验证成本从"人肉对话多轮"降到"一条命令"**。起因是排查 `identity` 空快照那轮消耗了大量用户时间与 token，而根因本可在本地 90 秒内定位。**交付**：① `dev-server/tools/probe-mvu-update.py` —— 两阶段复现真机会话形态（先只 seed 开场白让 MVU 把变量初始化到楼层 0，再用它重建 3 层会话）→ 等 `mvu_zod` 注册 → 预置并勾选 `#mvu_notification_error`（否则它的校验失败**完全静默**）→ 派发 `MESSAGE_RECEIVED(2)` → 逐条 patch 断言，退出码 0/1 可直接作回归项；② `.docs/plugin-system/插件开发_JS-Slash-Runner/MVU变量更新-本地自测方法.md` —— 原理、**三个必须遵守的前提**、观测点解读、失败对照表、范例与局限。**三条前提（都是踩过的坑）**：I 必须两阶段 seed（`At(e)=Ct(e)` 用 `chat.slice(0,e)`，**不含第 e 层**，变量只在最新层会让 `Yt` 直接早退）；II 必须等 `mvu_zod` 注册（它 import 4 个 npm 模块、约 30s；只等 `window.Mvu` 会走"**无 zod**"路径 ⇒ 命令由 MVU 主循环应用 ⇒ 变量正常更新 ⇒ **误判为已修复**）；III 必须打开它的静默告警开关。**反向对照**：把 `identity` 改回空快照并重建后，同一命令给出三条 `✗` + `[warn] ✖ Invalid input: expected object, received undefined → 路径: user` + 退出码 1；修复版则 `✓✓✓` + 退出码 0 ⇒ 该脚本**随故障变红，不是恒真断言**。 |
...
| # | 问题 | 证据 | 建议 |
|---|---|---|---|
| K1 | **安全头曾整组不下发** —— 2026-09-16 **已修**；CSP 的 `script-src` 仍有一个**已知收窄缺口** | 根因：nginx 语义「子块一旦出现 `add_header`，父级所有 `add_header` 全被覆盖」，而 `location = /index.html`（SPA 回退的落点）自带 Cache-Control/Pragma/Expires ⇒ server 级的 5 个安全头在该 location 下**全部丢失**。实测（真实 nginx 容器 + 生产产物）：修前 `GET /` 安全头 **0/5**，修后 **5/5**（`/index.html`、任意 SPA 路由同样 5/5）；`dev-server` 模板同源缺陷也一并修好（dev 现 6/6，且补上了它此前漏掉的 `Permissions-Policy`）。**收窄缺口**：`script-src` 必须保留 `'unsafe-inline'` —— S-a4 沙箱实验实证：下发 `script-src 'self'`（无 inline）会让 srcdoc 的**内联**装配脚本被拒执行（宿主 iframe 的 console 连「宿主脚本构建」那行都没有、MVU 永不就绪、变量为空）。⇒ 要收紧到 `'self'` 必须先按 D13② 迁移脚本载体（把内联脚本外置成同源文件），那是独立改动 | **A 部分已完成**；**B 部分（收紧 `script-src`）依赖载体迁移**，单独立项。防漂移断言：`verify-prod-artifacts.py --only K1`（凡自带 `add_header` 的 location 都必须带齐安全头） |
...
| K9 | **MVU 的空聊天初始化是「一次性且不可逆」的** | bundle 的 initvar 路径：`At(getLastMessageId()+1)` 取不到数据时 `console.error(noMessagesLog)`（中文「不存在任何一条消息，退出」）并以 `{display_data:{},initialized_lorebooks:{},stat_data:{},delta_data:{},schema:{type:'object',properties:{}}}` 兜底，随后把 `initialized_lorebooks[卡名]` 记下；此后 `if(!_.has(e.initialized_lorebooks,t))` 恒假 ⇒ initvar **永远不再执行**。真机日志实证（v11-1542，07:53:02Z 快照：`chatReady:false, messages:0` + console「不存在任何一条消息，退出」） | 已从源头避免：插件侧「等 chatReady 才装配」+「初始化窗口阻塞输入」（本轮）。**若某会话已经踩过坑（变量恒空）**：需要人工清掉该会话楼层变量里的 `initialized_lorebooks` 记名后重开才会重跑 initvar —— **本轮未做自动自愈**（避免误删真实数据），如需请单独立项 |
